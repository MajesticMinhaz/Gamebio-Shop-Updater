...
async def csv_file_format() -> None:
    async with aiofiles.open("./shop_menge_liefezeit_output.csv", "r") as wrong_format_csv_file:
        file_data = await wrong_format_csv_file.read()
        file_data = file_data.replace("|", ",")
        file_data = file_data.replace('"XTSOL",', "")
        file_data = file_data.replace('XTSOL,', "")
        async with aiofiles.open("shop_menge_liefezeit_output_format.csv", "w") as write_correct_csv_file:
            correct_data = await write_correct_csv_file.write(file_data)
            print(correct_data)
            print("shop_menge_liefezeit_output_format.csv file created.")
...
async def create_all_schema() -> None:
    ...
    update_data_set = numpy.array(list(csv.reader(open("./shop_menge_liefezeit_output_format.csv", "r"), delimiter=","))).astype("str")
...
if __name__ == "__main__":
    ...
    if len(all_schemas) >= 500:
        ...
        index_number = numpy.append(index_number, 0)

    first_index = 0
    last_index = 0
    headers = {'content-type': 'application/json', 'Authorization': f'Basic {config["BASIC_AUTH_TOKEN"]}'}
    if len(index_number) > 1:
        ...
        request_data = json.dumps(all_schemas.tolist())
        response = requests.request(
            method="PUT",
            url=config["URL"],
            headers=headers,
